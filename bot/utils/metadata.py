...
async def set_flac(data: TrackMetadata, handle):
    ...
    await set_cover(handle, data)
    handle.save()
    return True

async def set_mp3(data: TrackMetadata, handle):
    # ID3
    if handle.tags is None:
            handle.add_tags()
...
async def set_m4a(data: TrackMetadata, handle):
    if handle.tags is None:
        handle.add_tags()
...
async def set_cover(handle, metadata):
