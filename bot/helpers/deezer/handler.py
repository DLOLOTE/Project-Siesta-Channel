...
async def start_playlist(playlist_id, user):
    ...
    playlist_sort = False if bot_settings.upload_mode == 'Telegram' else bot_settings.playlist_sort
    ...
    if bot_settings.playlist_conc:
        ...
        i = 0
        if bot_settings.playlist_zip: upload = False
        for track in play_meta['tracks']:
            await progress_message(i, len(play_meta['tracks']), update_details)
            await start_track(track['itemid'], user, track, upload, playlist_folder, bot_settings.disable_sort_link)
            i+=1
